...
interface IRewardsDistributor_1_fsUSDe_fARB {
    function SYSTEM_PRECISION() external view returns (uint256);
    function collateralType() external view returns (address);
    ...
    function payout(uint128, uint128 poolId_, address collateralType_, address payoutTarget_, uint256 payoutAmount_) external returns (bool);
